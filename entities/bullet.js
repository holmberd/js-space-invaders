/** 
 * Bullet Module
 * Contains the Bullet Object Constructor and the helper function for creating the bullets.
 */
...
Bullet.prototype.update = function bulletUpdate (scope) {
  ...
  if (this.inBoundary(scope)) {
    // Bullet direction is velocity dependent pos/neg
...
Bullet.prototype.collision = function (entity) {
  if (entity.group === 'player' && this.pc) {
    ...
    this.kill();
  } else if (entity.group === 'invader' && !this.pc) {
    // (!this.pc) Invader's bullets can't collide with invader
    this.kill();
...
Bullet.prototype.reset = function () {
  ...
  this.pc = false;
  // Restore velocity to default negative number
